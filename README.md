...
実装中です。現在は中間 JSON のスキーマ、プロジェクト作業ディレクトリ、CLI 骨格（`init` / `doctor`）、音声前処理（`audio-fix`）が動きます。
...
残りの処理段（`transcribe` / `correct` / `detect` / `render` / `report`）はサブコマンドとして登録済みですが中身は未実装です。設計は `docs/design.md`、検証計画は `docs/verification-plan.md`、調査根拠は `docs/research/feasibility-report.md` を参照してください。
