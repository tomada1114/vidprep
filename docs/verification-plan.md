...
**限界と運用**: ASR の非決定性・境界の音響変化により誤検知（false positive）は起こりうる。そのため v1 導入時は**警告扱い（advisory）で開始**し、§10 のフォールトインジェクション（故意の語中カット）で検出力を、正常カットで誤検知率を確認したうえで、pass/fail ゲートに昇格させる方針とした（**この昇格は #32 で完了した。下記「昇格条件の再定義」以降**）。コストは ASR 1 回分（ゴールデンで数十秒〜数分）で許容範囲。

**実装上の判断（#11）**:

- **欠落 hunk は `difflib` の `delete` オペコードのみ**を数える（**tomada 承認 2026-08-05、#32**）。`replace` は「再 ASR が何かを聞き取ったが読み違えた」＝本手法が相殺を狙っているモデルノイズそのものであり、「期待側にあり再 ASR 側にない」（§8.1-4 の文言）には当たらないため対象外とする。語中カットは音声そのものが消えるので純粋な削除として現れる（下記実測で確認）。この規則の代償は §11 の CER 実測がそのまま示している — 辞書校正が期待側だけを直した 8 セグメントは全て `replace` に落ち、CER を 7.99% まで押し上げながら hunk は 0 件のままだった。**判定に効く数（hunk）が校正の影響を受けず、参考値（CER）だけが動く**のは、delete-only がまさに狙った挙動である
...
- **advisory / gate の切り替えは `profile.json` の `render.verify_asr_mode`**（#11 実装時の既定は `advisory`、#32 で `gate` へ昇格）。`gate` では境界欠落フラグ 1 件以上で exit 3
...
**中間結論（2026-08-04、advisory 据え置き）**: 検出力・位置精度は実証され、誤検知率も 0 だったが、根拠が**ゴールデン 1 本・各 1 回の実行**に留まり、本手法の既知の弱点である ASR の非決定性（同一入力を複数回走らせたときのばらつき）を一度も標本化していなかったため、この時点では advisory に据え置いた。旧昇格条件は「① 同一 render に対する照合 3 回以上でフラグ毎回 0 件、② 性質の異なる素材をもう 1 本追加してそちらでも 0 件」。

**再現性の実測（#32、2026-08-05）**: 2026-08-04-03 のゴールデンラン出力（`fixtures/golden/out/output.mp4`）に対し、**再レンダリングせずに**照合だけを 3 回走らせた。ASR の非決定性のみを標本化するのが目的なので、期待テキスト・カット・境界はどの回も同一である。照合だけを繰り返す入口は CLI に無い（`render --verify-asr` は毎回レンダリングし直す）ため、`render.subject()` が作った `verify.Subject` を `verify.run_verify_asr()` に 3 回渡す使い捨てスクリプトで測った。生の 3 回分は `fixtures/verify-repeat-2026-08-05.json`（git 管理外）。

| 回 | 境界フラグ | 欠落 hunk | 期待 / 再 ASR 文字数 | 境界数 | グローバル CER | 所要 |
|---|---|---|---|---|---|---|
| 1 | **0** | 0 | 1101 / 1074 | 52 | 7.99% | 82.8s |
| 2 | **0** | 0 | 1101 / 1074 | 52 | 7.99% | 42.0s |
| 3 | **0** | 0 | 1101 / 1074 | 52 | 7.99% | 41.9s |

- **3 回とも報告値が全一致**した（1 回目だけ所要が倍なのはモデルのロード分）。advisory 据え置きの根拠だった「同一入力でも再 ASR の出力が揺れる」ばらつきは、whisper.cpp large-v3-turbo + Silero VAD をこの構成で走らせるかぎり**観測されなかった**。文字数と hunk 数まで一致しているので、揺れがあったとしても判定に届く大きさではない
- 誤検知率（REQ-013）は 0/52 が 3 回。#9・2026-08-04-03・§11 の切り分け 2 ラン（2026-08-05 と 2026-08-05-02）を合わせると、実素材の正常 render に対する照合は**計 7 回・通算 0 / 364 境界**でフラグ 0 件

**昇格条件の再定義（tomada 判断、2026-08-05）**: 旧条件 ② は、ゴールデンが 1 本しかない現状では**満たしようがない**（＝永久に昇格しない）条件だった。素材追加は §2 の既知の制約であって本チェックの検証項目ではないため、昇格条件から外し、判定可能な 3 条件に置き換える:

1. **検出力**: 実素材の語中カット注入で境界フラグ 1 件が立ち、`cut_id` と欠落文字列が注入箇所と一致すること → §8.1 実測表で実証済み。回帰は §10 ケース #2 が CI で守る
2. **非決定性**: 同一 render に対する照合を 3 回以上繰り返し、境界フラグが毎回 0 件であること → 上表で実証済み
3. **誤検知**: 実素材の正常 render に対する誤検知率（フラグ数 / 境界数）が 0 であること → 通算 0 / 364 境界（7 回）で実証済み

**結論（2026-08-05、gate へ昇格）**: 3 条件をすべて満たしたので、`render.verify_asr_mode` の既定を **`advisory` → `gate`** に切り替える（`src/vidprep/profiles/default.json` と `RenderProfile` の既定値）。v1 は gate で出荷する。素材多様性は昇格の前提条件ではなく**再点検のトリガ**として残す: 2 本目の素材を追加した時点でその素材の正常 render に対し条件 1〜3 を測り直し、誤検知が出たら ±2 秒窓の見直しか advisory への差し戻しを検討する。

**gate の運用**:

- `vidprep render --verify-asr` は境界フラグ 1 件で exit 3。プロジェクト単位で `profile.json` の `render.verify_asr_mode` を `advisory` に戻せば従来どおり警告のみになる。既存プロジェクトの `profile.json` は `init` 時にコピーされた値をそのまま保持するため、影響を受けるのは**新規プロジェクトと既定値を明示していない呼び出し**だけである
- gate 判定は CLI 層（`cli.py`）にあり、ライブラリの `render.run_render` は結果を返すだけである。したがって `just golden`（`scripts/golden_run.py`）はフラグが立っても止まらず、`warnings.json` に警告として残り、`just golden-diff` が `verify_asr.near_boundary_flags` の増加に ⚠ を付ける。ゴールデンランで人が見る面はこの 2 つ
- 誤検知が出たときの復旧手順は「`report/boundary_digest.mp4` で該当境界を試聴 → 問題なければ profile を `advisory` にして再実行」。ローカル実行のツールなので影響範囲はその 1 実行に閉じる

**検証手順**
```
vidprep render --verify-asr        # 再文字起こし照合つき（既定 gate: フラグ 1 件で exit 3）
...
特に #2 は再文字起こし照合（§8.1）の**検出力の実証**であり、advisory → ゲート昇格の判断材料になった（#32 の昇格条件 1。昇格後は、検出力を弱める変更が入れば CI で落ちる回帰テストとして働く）。
...
just golden --skip correct   # 1 段だけ外した計測ラン（#32 の CER 切り分けで使用）
```
...
- **再文字起こし照合（§8.1）は境界欠落フラグ 0 / 52 境界**（advisory）。グローバル CER は 7.99%（期待 1101 文字 / 再 ASR 1074 文字）で、#9 の 0.28% より高い。**間に `correct` が入ったため**で、辞書が原稿側だけを直す（「リズーム」→ `resume` 等）一方、再 ASR は同じ聞き違いを繰り返すぶん差分として残る（**#32 で実測確認。下記「CER 乖離の要因切り分け」**）。CER は判定に使わない参考値（REQ-007）であり、フラグは 0 件のまま
...
**CER 乖離の要因切り分け（#32、2026-08-05、実測済み）**: 上の 7.99% が `correct` 由来だという推定を、**`correct` だけを外した同一構成のラン**（`just golden --skip correct`、`fixtures/runs/2026-08-05/`）で確かめた。

| 条件 | 期待 / 再 ASR 文字数 | 欠落 hunk | 境界フラグ | グローバル CER |
|---|---|---|---|---|
| 6 段（`correct` あり、`fixtures/runs/2026-08-04-03/`） | 1101 / 1074 | 0 | 0 / 52 | **7.99%** |
| 5 段（`correct` を `--skip`、`fixtures/runs/2026-08-05/`） | 1079 / 1074 | 0 | 0 / 52 | **1.48%** |
| 6 段 再走（対照、`fixtures/runs/2026-08-05-02/`） | 1101 / 1074 | 0 | 0 / 52 | **7.99%** |

- **推定は当たっていた**。辞書校正を外すと CER は 7.99% → **1.48%（-6.51pt）** に落ちる。差は期待テキストの側だけが直った 22 文字（8 セグメント、「クロードコード」→ `Claude Code` ほか）に対応し、再 ASR は同じ聞き違いを繰り返すのでそのぶんが丸ごと `replace` として残る
- 対照として同じ日に 6 段を走らせ直すと 7.99%（1101 / 1074）が**桁まで再現**した。したがって 6.51pt は実行間のドリフトではなく `correct` の有無そのものに帰属する。再 ASR 側は 3 条件とも 1074 文字だった（別々に再エンコードした `out/output.mp4` に対する別々の ASR 実行であり、動いたのは期待テキストの側だけである）
- したがってこれは**校正が効くほどグローバル CER が上がるという設計上の性質**であり、劣化ではない。判定に使う数（欠落 hunk・境界フラグ）は両条件とも 0 のまま動かない — `delete` オペコードだけを数える §8.1 の規則が意図どおり働いている証拠でもある
- 残る 1.48%（約 16 文字）は ASR 自身のノイズ水準。#9 の 0.28% とは直接比べられない（#9 は #30 修正前の別 render・別 transcript に対する測定）。**この 1.48% ではなく「フラグ 0 件」がゴールデンランの合否**であり、CER が動いたときに見るのは「校正・辞書を変えたか」であって「認識が悪化したか」ではない
- 2 ランの `just golden-diff` が出す差は 4 行だけで、`verify_asr.expected_chars 1079 → 1101` / `verify_asr.global_cer 0.015 → 0.080` / `subtitles.warnings.max_cps 0 → 1` / 所要秒である。**`correct` が動かす面がこの 3 つに限られている**ことがそのまま読める（`max_cps` の 1 件は s0011。「クロードコード」7 文字が `Claude Code` 11 文字になり、同じ表示時間で読ませる文字数が増えるためで、上の #30 記録にある 8.65 文字/秒の警告は**辞書校正の副作用**だと分かる）
- 5 段ランは基準ラン（baseline）ではない。`summary.json` に `"skipped": ["correct"]` が残るので、`just golden-diff` で拾ったときに素性が分かる
