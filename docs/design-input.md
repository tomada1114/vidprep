...
2. **すべての中間データは機械可読 JSON。** AI エージェントのスキルと人間が同じファイルを編集でき、各コマンドは「JSON を読んで JSON を書く」に統一する。CLI 本体に AI 依存は入れない。
...
原尺素材 (mp4)
 ...
 │                 └─ ★レビューゲート（人間 or agent skill が cuts.json を編集）
 └─ [render]
...
- Agent 連携: ほぼ不要。処理前後の LUFS 統計を JSON で出し、結果確認のみ
...
- Agent 連携: transcript.json がすべての起点。スキーマに `id` / `words` / `source` / `edits[]`（編集履歴）を持たせる
...
- Agent 連携: LLM 校正はスキルの仕事。返答は「id 集合の一致・件数一致・タイムスタンプ不変」を機械検証してから適用（捏造防止）
...
- Agent 連携: cuts.json に `reason`（silence / filler / manual）と `confidence` を持たせ、スキルが採用/却下を編集できる形にする
...
- Agent 連携: telops.json（どのセグメントに・どのプリセットで・何秒表示）をスキルが書き、CLI がレンダリング。「どこにテロップを入れるか」の判断こそ AI の仕事
...
- Agent 連携はリポジトリ内 `.claude/skills/` を正本として置く。Codex 側には `.agents/skills/` の symlink を用意し、スキルは CLI を呼び中間 JSON を読み書きするだけ
